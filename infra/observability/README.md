...
traces). Full design & RCA workflow: the **## Observability** section in the repo-root `CLAUDE.md`.
...
There is no Grafana MCP wired — prod RCA is done against Cloud Logging via the `gcloud` CLI (see the
**## Observability** section in the repo-root `CLAUDE.md`). For this optional local stack, just open Grafana at http://localhost:3000
